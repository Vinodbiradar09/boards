...
export default function OrganizationSetupForm({
  ...
  const hasValidEmails = () => {
    return (
      teamEmails.filter((email) => email.trim() && email.includes("@")).length >
      0
    );
  };
  ...
  return (
    <form onSubmit={handleSubmit} className="space-y-6 dark:text-zinc-400">
      ...
      <div className="space-y-4">
        ...
        <Button onClick={addEmailField} type="button" variant="outline" className="w-full">
      ...
      <Button type="submit" className="w-full" disabled={isSubmitting}>
          {isSubmitting ? "Creating..." : hasValidEmails() ? "Save and Send Invites" : "Save"}
      </Button>
    </form>
  );
}
